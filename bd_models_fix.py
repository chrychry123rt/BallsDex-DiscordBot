...
class BallInstance(models.Model):
    ...
    def countryball(self) -> Ball:
        # Temporary fix: Try cached first, fallback to uncached
        cached = balls.get(self.ball_id, None)
        if cached:
            return cached
        # If not in cache, return from cache dict or self.ball
        # This avoids the async context issue
